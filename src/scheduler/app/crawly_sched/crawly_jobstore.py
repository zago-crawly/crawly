...
try:
    ...
    raise ImportError('CrawlyJobStore requires SQLAlchemy installed')


class CrawlyJobStore(BaseJobStore):
    """
    ...
    def __init__(self, url=None, engine=None, tablename='crawly_jobs', metadata=None,
                 pickle_protocol=pickle.HIGHEST_PROTOCOL, tableschema=None, engine_options=None):
        super(CrawlyJobStore, self).__init__()
        ...
        self.jobs_t = Table(
            ...
            Column('template_uuid', String, nullable=False, index=True),
            Column('schema_uuid', String, nullable=False, index=True),
            schema=tableschema
        )

    def start(self, scheduler, alias):
        super(CrawlyJobStore, self).start(scheduler, alias)
    ...
    def lookup_job_with_condition(self, condition):
        selectable = select(self.jobs_t.c.job_state).where(self.jobs_t.c.id == job_id)
        with self.engine.begin() as connection:
            job_state = connection.execute(selectable).scalar()
            return self._reconstitute_job(job_state) if job_state else None
    ...
    def get_jobs_by_template(self, template_uuid: str):
        return self._get_jobs(self.jobs_t.c.template_uuid == template_uuid)
    ...
    def add_job(self, job, template_uuid: str, schema_uuid: str):
